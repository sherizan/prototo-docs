...
Tap to test interactions. Watch it update after every prompt. When it looks right, share it — for now see [Share via GitHub](/tips/share-github) until built-in sharing ships.
...
GitHub (workaround)  → they clone & run       (see Share via GitHub)
```
