...
That's it. You don't usually run `proto login` directly — [`npx proto share`](/sharing/share-link) and [`npx proto record`](/studio/recording) sign you in automatically the first time you use them.
...
<Note>
There's no `proto logout` command yet — removing that file is how you sign out. The next `npx proto share` or `proto login` signs you back in.
</Note>
